...
*/
async function stringToPublicKeyForEncryption(pkeyBase64: string): Promise<CryptoKey> {
    try {
        const keyArrayBuffer: ArrayBuffer = this.base64StringToArrayBuffer(pkeyBase64)
...
The SubtleCrypto imposes to use the "spki" format for exporting public keys.
*/
async function stringToPublicKeyForSignature(pkeyBase64: string): Promise<CryptoKey> {
    try {
        const keyArrayBuffer: ArrayBuffer = this.base64StringToArrayBuffer(pkeyBase64)
        const key: CryptoKey = await window.crypto.subtle.importKey(
            "spki",
...
*/
async function stringToPrivateKeyForEncryption(skeyBase64: string): Promise<CryptoKey> {
    try {
        const keyArrayBuffer: ArrayBuffer = this.base64StringToArrayBuffer(skeyBase64)
        const key: CryptoKey = await window.crypto.subtle.importKey(
            "pkcs8",
            keyArrayBuffer,
            {
...
The SubtleCrypto imposes to use the "pkcs8" ?? format for importing public keys.
*/
async function stringToPrivateKeyForSignature(skeyBase64: string): Promise<CryptoKey> {
    try {
        const keyArrayBuffer: ArrayBuffer = this.base64StringToArrayBuffer(skeyBase64)
        const key: CryptoKey = await window.crypto.subtle.importKey(
            "pkcs8",
...
The SubtleCrypto imposes to use the "spki" format for exporting public keys.
*/
async function privateKeyToString(key: CryptoKey): Promise<string> {
...
async function generateAssymetricKeysForEncryption(): Promise<CryptoKey[]> {
...
async function generateAssymetricKeysForSignature(): Promise<CryptoKey[]> {
    const keypair: CryptoKeyPair = await window.crypto.subtle.generateKey(
        {
            name: "RSASSA-PKCS1-v1_5",
            modulusLength: 2048,
...
async function encryptWithPublicKey(publicKey: CryptoKey, message: string): Promise<string> {
...
async function signWithPrivateKey(privateKey: CryptoKey, message: string): Promise<string> {
...
async function decryptWithPrivateKey(privateKey: CryptoKey, message: string): Promise<string> {
    try {
        const decrytpedMessageAB: ArrayBuffer = await
            window.crypto.subtle.decrypt(
                { name: "RSA-OAEP" },
                privateKey,
                this.base64StringToArrayBuffer(message)
            )
...
async function verifySignatureWithPublicKey(publicKey: CryptoKey, messageInClear: string, signedMessage: string): Promise<boolean> {
    try {
...
async function symmetricKeyToString(key: CryptoKey): Promise<string> {
...
async function stringToSymmetricKey(skeyBase64: string): Promise<CryptoKey> {
    try {
...
async function encryptWithSymmetricKey(key: CryptoKey, message: string): Promise<string[]> {
...
async function decryptWithSymmetricKey(key: CryptoKey, message: string, initVector: string): Promise<string> {
