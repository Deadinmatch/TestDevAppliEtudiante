...
function decrypt(text: string, key: string): string {
  ...
  for (let char of text) {
    if (/[a-zA-Z]/.test(char)) {
      ...
      if (char.match(/[a-z]/)) {
        if (shiftedCharCode > 122) {
          shiftedCharCode -= 26;/* tsc --inlineSourceMap  true -outFile JS/messenger.js src/libCrypto.ts src/messenger.ts --target es2015 */
...
async function deleteForAll() {
  ...
  nonceA = generateNonce();
  messageStatic = selectedMessageId + "d&d";
  fileAttente.addAttente(nonceA, receiverStatic, selectedMessageId);
  await deroulerProtocole("", false);
  deleteForMe();
  isDeleteForAll = false;
}
